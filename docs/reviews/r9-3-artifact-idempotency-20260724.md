...
The regression test `test_sequential_final_pdf_replays_are_side_effect_free` takes its response, renderer, PDF/manifest, generation, DB-count, and audit-count baseline immediately after the first publication, then compares each of three sequential replays independently against it. `test_tampered_existing_artifact_replay_fails_closed` proves one real renderer call for the first publish, then a 409 response with no further renderer call, repair, duplicate artifact, or additional export audit event for a tampered existing PDF.
