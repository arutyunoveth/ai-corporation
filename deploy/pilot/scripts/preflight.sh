#!/usr/bin/env bash
set -euo pipefail
[[ "${ARVECTUM_IMAGE_REVISION:-}" =~ ^[0-9a-f]{40}$ ]] || { echo "set exact 40-character ARVECTUM_IMAGE_REVISION" >&2; exit 2; }
[[ -n "${ARVECTUM_IMAGE_VERSION:-}" && "${ARVECTUM_IMAGE_VERSION}" != unreleased && "${ARVECTUM_IMAGE_VERSION}" != *placeholder* ]] || { echo "set R7-specific ARVECTUM_IMAGE_VERSION" >&2; exit 2; }
