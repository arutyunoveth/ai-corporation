...
run_bad=re.compile(r'[\s/\\`$;|&<>(){}\[\]"\']')
path_bad=re.compile(r'[\s\\`$;|&<>(){}\[\]"\']')
if not isinstance(run,str) or not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]{2,127}',run) or '..' in run or run_bad.search(run): raise SystemExit('unsafe accepted run ID')
path_obj=pathlib.PurePosixPath(path) if isinstance(path,str) else None
if not isinstance(path,str) or path.startswith('/') or '//' in path or path_bad.search(path) or path_obj.parts[:3] != ('data','demo','exports') or len(path_obj.parts) != 4 or path_obj.as_posix() != path or not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]*\.pdf',path_obj.name): raise SystemExit('unsafe accepted PDF path')
