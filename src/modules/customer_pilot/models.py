...
class PilotProject(UUIDPrimaryKeyMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("customer_id", "internal_slug"), Index("ix_pilot_projects_customer", "customer_id"))
...
class ProcurementCase(UUIDPrimaryKeyMixin, Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=utcnow)
    __table_args__ = (UniqueConstraint("customer_id", "project_id", "procurement_number"), Index("ix_procurement_cases_customer_project", "customer_id", "project_id"), Index("ix_procurement_cases_status", "status"))
...
class PilotReview(UUIDPrimaryKeyMixin, Base):
    ...
    run_id: Mapped[str] = mapped_column(String(36), ForeignKey("tender_analysis_runs.id"), nullable=False, unique=True)
    ...
    __table_args__ = (Index("ix_pilot_reviews_customer_case", "customer_id", "procurement_case_id"),)
...
class PilotFeedback(UUIDPrimaryKeyMixin, Base):
    ...
    __table_args__ = (Index("ix_pilot_feedback_customer_case", "customer_id", "procurement_case_id"),)
...
class PilotAuditEvent(UUIDPrimaryKeyMixin, Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=utcnow)
    __table_args__ = (Index("ix_pilot_audit_customer_created", "customer_id", "created_at"),)
