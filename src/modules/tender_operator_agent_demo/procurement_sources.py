...
def get_procurement_source_descriptors() -> list[ProcurementSourceDescriptor]:
    ...
    return [
        ...
        ProcurementSourceDescriptor(
            ...
            enabled=True,
            read_only=True,
            note="Публичный fallback: открыть поиск ЕИС по 44-ФЗ, выбрать закупку и запустить обработку из интерфейса.",
        ),
        ProcurementSourceDescriptor(
            ...
            enabled=True,
            read_only=True,
            note="Публичный fallback: открыть поиск ЕИС по 223-ФЗ, выбрать закупку и запустить обработку из интерфейса.",
        ),
        ProcurementSourceDescriptor(
            code="public_eis_html_capital_repair",
