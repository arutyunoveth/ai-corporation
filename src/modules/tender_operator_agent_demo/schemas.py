...
class StopFactorSeverity(StrEnum):
    INFO = "info"
    WARNING = "warning"
    CRITICAL = "critical"


class StopFactor(APIModel):
    code: str
    title: str
    severity: StopFactorSeverity
    evidence: str
    source: str


class ResellerDecisionLabel(StrEnum):
    GO = "GO"
    NO_GO = "NO_GO"
    NEEDS_REVIEW = "NEEDS_REVIEW"
...
class ResellerTriageReport(APIModel):
    decision_label: ResellerDecisionLabel
    ...
    tender_card: TenderCard | None = None
    line_items: list[LineItem] = Field(default_factory=list)
    has_line_items: bool = False
    export_report_url: str | None = None
    details_status: str = "search_only"
    documents_status: str = "not_requested"
    analysis_completeness: str = "search_only"
    documents_count: int = 0
    downloaded_documents_count: int = 0
    parsed_documents_count: int = 0
    line_items_count: int = 0


class ResellerTenderSearchResult(APIModel):
    procurement_id: str
    title: str
    customer_name: str
    initial_price: float | None = None
...
class ResellerSearchRequest(APIModel):
    query: str
    law: str | None = None
...
class ResellerSearchResponse(APIModel):
    query: str
    filters: dict[str, Any] = Field(default_factory=dict)
    total_results: int | None = None
    total_results_label: str | None = None
    total_count_kind: TotalCountKind = TotalCountKind.EXACT
    results: list[ResellerTenderSearchResult]
    freshest: ResellerTenderSearchResult | None = None
...
class ResellerSearchAndTriageResponse(APIModel):
    search: ResellerSearchResponse
    triage: ResellerTriageReport | None = None
    analysis_limit_notice: str
    status: str
    synthetic_used: bool = False
