...
def _render_canonical_report_html(model: dict[str, Any]) -> str:
    ...
    compatibility_table = (f"<div class='scroll'><table><thead><tr>{''.join(f'<th>{esc(column)}</th>' for column in compatibility.get('spec_columns', []))}</tr></thead><tbody>{compatibility_rows}</tbody></table></div>" if compatibility.get("spec_rows") else "")
