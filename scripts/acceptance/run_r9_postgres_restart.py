...
SUCCESS = "R9_2_POSTGRESQL_RESTART_SMOKE_PASS_LOCAL_FAIL_CLOSED_EVIDENCE"
FILES = ("postgres-restart-result.json", "postgres-lifecycle.json", "application-lifecycle.json", "volume-identity.json", "http-snapshots.json", "database-snapshots.json", "filesystem-snapshots.json", "verifier-results.json", "postgres-identity.json", "backend-first.log", "backend-second.log", "commands.log", "cleanup.json", "SHA256SUMS")
...
def sql_identity(env: dict[str, str], commands: list[dict[str, Any]], secrets_map: dict[str, str], check: bool = True) -> dict[str, Any] | None:
    code = """import json,os
...
def wait_for_postgres(project: str, env: dict[str, str], commands: list[dict[str, Any]], secrets_map: dict[str, str]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    while time.monotonic() < deadline:
        ...
        if identity["running"] and identity["health"] == "healthy" and probe:
...
def assertions(data: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "application_stopped_before_postgres_stop": first.get("process_exited") is True and first.get("exited_at", "") <= data["postgres_lifecycle"].get("stop_requested_at", ""),
        "postgres_stop_exit_zero": data["postgres_lifecycle"].get("stop_exit_code") == 0,
        "postgres_unavailable_while_stopped": data["postgres_lifecycle"].get("unavailable_probe_return_code") != 0,
        ...
        "postgres_container_id_preserved": pgpre.get("container_id") == pgpost.get("container_id"),
        "postgres_started_at_changed": pgpre.get("started_at") != pgpost.get("started_at"),
        "postgres_volume_name_preserved": volume_pre.get("volume_name") == volume_post.get("volume_name"),
        "postgres_volume_mountpoint_preserved": volume_pre.get("volume_mountpoint") == volume_post.get("volume_mountpoint"),
        "postgres_volume_created_at_preserved": volume_pre.get("volume_created_at") == volume_post.get("volume_created_at"),
        "postgres_mount_binding_preserved": (volume_pre.get("mount_source"), volume_pre.get("mount_destination")) == (volume_post.get("mount_source"), volume_post.get("mount_destination")),
        "postgres_system_identifier_preserved": data.get("sql", {}).get("pre", {}).get("system_identifier") == data.get("sql", {}).get("post", {}).get("system_identifier"),
        "postgres_database_identity_preserved": all(data.get("sql", {}).get("pre", {}).get(k) == data.get("sql", {}).get("post", {}).get(k) for k in ("database", "user")),
        ...
        "first_application_exited_before_postgres_stop": first.get("exited_at", "") <= data["postgres_lifecycle"].get("stop_requested_at", ""),
        "second_application_started_after_postgres_ready": second.get("start_requested_at", "") >= data["postgres_lifecycle"].get("sql_ready_at", ""),
        ...
        "no_post_restart_mutations": data.get("post_restart_mutations", 1) == 0,
...
def main() -> int:
    ...
    commands: list[dict[str, Any]] = []; data: dict[str, Any] = {"state": {}, "application": {"first": {}, "second": {}}, "postgres_lifecycle": {}, "postgres": {}, "volume": {}, "sql": {}, "http": {}, "database": {}, "filesystem": {}, "verifier": {}, "cleanup": {}, "cleanup_errors": [], "post_restart_mutations": 0}
    first = second = None
    try:
        compose = ["docker", "compose", "-p", project, "-f", str(COMPOSE)]
        ...
        data["postgres_lifecycle"]["stop_requested_at"] = utcnow(); stopped = run_command(compose + ["stop", "postgres"], env, commands, secrets_map); data["postgres_lifecycle"]["stop_exit_code"] = stopped.returncode; data["postgres_lifecycle"]["stopped_at"] = utcnow(); data["postgres"]["stopped"] = inspect_postgres(project)
        data["postgres_lifecycle"]["unavailable_probe_at"] = utcnow(); unavailable = sql_identity(env, commands, secrets_map, check=False); data["postgres_lifecycle"]["unavailable_probe_return_code"] = 0 if unavailable else 1
        data["postgres_lifecycle"]["start_requested_at"] = utcnow(); started = run_command(compose + ["start", "postgres"], env, commands, secrets_map); data["postgres_lifecycle"]["start_exit_code"] = started.returncode; data["postgres"]["post"], data["sql"]["post"] = wait_for_postgres(project, env, commands, secrets_map); data["volume"]["post"] = data["postgres"]["post"]; data["postgres_lifecycle"]["healthy_at"] = utcnow(); data["postgres_lifecycle"]["sql_ready_at"] = utcnow(); data["postgres_lifecycle"]["post_identity_at"] = utcnow(); data["revision"]["post"] = revision(env, commands, secrets_map)
        ...
        data["http"]["post"] = fetch_http_snapshot(base, data["state"]); data["database"]["post"] = snapshot_database(env); data["filesystem"]["post"] = snapshot_filesystem_stable(data_root); data["verifier"]["post"] = fetch_verifier_snapshot(env, data["state"])
        ...
        write_json(evidence / "postgres-restart-result.json", sanitize_value(result, secrets_map)); write_sums(evidence); exit_code = 0 if result["status"] == SUCCESS and all(result["assertions"].values()) and all((evidence / x).exists() for x in FILES) else 1
