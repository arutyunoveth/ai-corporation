...
def verify_file_sha(path: Path, expected_sha: str) -> bool: return path.is_file() and sha256_file(path) == expected_sha
def atomic_write_json(path: Path, payload) -> str:
    path.parent.mkdir(parents=True, exist_ok=True)
    fd, name = tempfile.mkstemp(prefix=f".{path.name}.", dir=path.parent)
    try:
        with os.fdopen(fd, "wb") as f:
            f.write(canonical_json_bytes(payload)); f.flush(); os.fsync(f.fileno())
...
capture_binding_snapshot = build_execution_binding
verify_execution_binding = build_execution_binding
